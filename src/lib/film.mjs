...
export const OVERLAY_TYPEN = [
  ...
  { id: 'riesen', name: 'Riesentitel (für hinter dich)', dauer: 5, felder: [['text', 'Ein Wort – oder zwei Zeilen mit |', 'text'], ['farbe', 'Farbe', 'select', ['', 'Weiß'], ['schwarz', 'Schwarz'], ['rot', 'Rot']], ['pos', 'Position', 'select', ['', 'Mitte'], ['oben', 'Oben'], ['unten', 'Unten']]], beispiel: { text: 'Heute' } },
...
OVERLAY_TYPEN.forEach(t => t.felder.push(['akzent', 'Akzentfarbe (*Wörter*, Kästen, Balken)', 'farbe'], ['akzent2', 'Zweitfarbe (~Wörter~)', 'farbe']));
...
export function overlayClip(o, i) {
  ...
  const q = { clip: t.id };
  for (const [k, , art] of t.felder) { let v = (o.werte || {})[k]; if (v === undefined || v === null || v === '') continue; if (art === 'farbe' && (!/^#[0-9a-f]{6}$/i.test(String(v)) || String(v).toUpperCase() === (k === 'akzent' ? '#E3000F' : '#FFD200'))) continue; if (art === 'lines') v = String(v).split('\n').map(x => x.trim()).filter(Boolean).join('|'); q[k] = String(v); }
...
export const BAUSTEINE_TEXT = OVERLAY_TYPEN.map(t => `- ${t.id} (${t.name}, ${t.dauer} s): ${t.felder.filter(([k]) => k !== 'anim').map(([k, l, art, ...opts]) => `${k}=${l}${art === 'select' ? ' [' + opts.map(([v, ol]) => `${v || 'leer'}=${ol}`).join(', ') + ']' : ''}`).join('; ')}`).join('\n')
  + `\nFarben: jeder Baustein hat akzent=Farbe der *hervorgehobenen* Wörter, Kästen und Balken als Hex (Standard #E3000F, SPD-Rot – meist so lassen; für Themen wie Umwelt oder Kinder darf es Grün, Gelb, Blau sein) und akzent2 für eine Zweitfarbe: im Text mit ~Wort~ markieren, im Wort-Popper eine Zeile mit ~ beginnen (Standard #FFD200, Gelb).`
