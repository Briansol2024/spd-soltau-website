...
import { FILM_ARTEN, FILM_STATUS, OVERLAY_TYPEN, ANIMATIONEN, FARBEN, overlayTyp, overlayClip, AUFTRAG, antwortLesen } from './lib/film.mjs';
...
export function makeFilm(ctx) {
  ...
  const bildUrl = (wert, vorschau) => { const m = /^material:(.+)$/.exec(wert); if (!m) return wert; const mat = st.material.find(x => x._id === m[1]); return (mat && mat.url) || (vorschau ? st.blobs[m[1]] || '' : ''); };
  ...
  // Wann läuft der Ablage-Dienst (GitHub Actions, öffentliches Repo) das nächste Mal? Er arbeitet zu Beginn jedes Laufs, Läufe etwa alle 5–6 Minuten
  const dienst = { at: 0, start: 0, status: '' };
  async function dienstStand() {
    if (Date.now() - dienst.at < 60000) return dienst;
    dienst.at = Date.now();
    try { const r = await (await fetch('https://api.github.com/repos/Briansol2024/spd-soltau-website/actions/workflows/push.yml/runs?per_page=1&exclude_pull_requests=true', { headers: { Accept: 'application/vnd.github+json' } })).json(); const run = r.workflow_runs?.[0]; if (run) { dienst.start = Date.parse(run.run_started_at || run.created_at); dienst.status = run.status; } } catch (e) { /* kein Netz, kein Stand */ }
    return dienst;
  }
  const dienstText = () => { if (!dienst.start) return ''; const naechster = dienst.start + 5.5 * 60000; const min = Math.round((naechster - Date.now()) / 60000); return min <= 0 ? 'Der Ablage-Dienst ist jeden Moment dran.' : `Nächster Durchlauf des Ablage-Dienstes in ca. ${min} Minute${min === 1 ? '' : 'n'}.`; };
  ...
  function bildFeld(p, k, l, wert, mehrfach) {
    ...
    const bilder = st.material.filter(m => (m.projektId === p._id || !m.projektId) && istBild(m));
    const thumbs = werte.map((w, i) => { const u = bildUrl(w, true); const mat = /^material:(.+)$/.exec(w); const m = mat && st.material.find(x => x._id === mat[1]); return `<span class="ow-thumb">${u ? `<img src="${esc(u)}" alt="">` : '<i>?</i>'}<small>${esc(m ? (m.titel || m.name || 'Bild') : w.replace(/^https?:\/\//, '').slice(0, 24))}${m && !m.url ? ' · wird bei Wix abgelegt' + (dienstText() ? ' – ' + dienstText().replace('Nächster Durchlauf des Ablage-Dienstes', 'nächster Lauf').replace('Der Ablage-Dienst ist jeden Moment dran.', 'jeden Moment') : '') : ''}</small><button type="button" class="linkbtn" data-bild-weg="${i}" aria-label="Entfernen">×</button></span>`; }).join('');
  ...
  function wireProjekt(v, p, overlays) {
    ...
    const bestellen = async (liste, titel, nurGruen, mitDrehplan, msgEl) => {
      ...
      const schritt = offen.length ? `Wartet auf ${offen.length === 1 ? 'ein Foto' : offen.length + ' Fotos'} – Wix legt ${offen.length === 1 ? 'es' : 'sie'} ab (meist unter 5 Minuten), danach startet der Agent von selbst.` : '';
    ...
    if (st.material.some(m => (m.projektId === p._id) && !m.url && m.status === 'wartet') && !dienst.start) dienstStand().then(() => { if (v.isConnected && st.tab === 'overlays') projekt(v, p._id); });
    // ---- Material ----
