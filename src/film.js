...
export function makeFilm(ctx) {
  ...
  function materialZeile(m) {
    ...
    const status = m.status === 'wartet' ? '<span class="small rot">wird abgelegt (bis 5 Min.) …</span>' : m.status === 'fehler' ? `<span class="small rot">Fehler: ${esc(m.fehler || '')}</span>` : !m.url && m.status === 'fertig' ? '<span class="small muted">nur für Mitglieder – kein öffentlicher Link</span>' : '';
