...
export const SCRIPTS = {
  ...
  kalender: S => ({ start: 'termine', steps: {
    android: [
      ...
      async () => { await S.tap('#kalender [data-kal-neu]'); await S.scroll('#kalender'); },
      async () => { await S.tap('#kalender .abo-karte [data-copy]'); },
      async () => { await S.overlay({ kind: 'dialog', top: '30%', width: 400, title: 'Nur am Rechner: calendar.google.com', text: '<div style="font-size:14px;color:#333;margin-bottom:10px">Weitere Kalender <b>+</b> → <b>Per URL</b></div><div style="border:1px solid #ccc;border-radius:6px;padding:10px;font-size:14px;color:#333">https://spd-soltau.de/assets/kalender/…ics</div>', buttons: ['Abbrechen', 'Kalender hinzufügen'], primary: 1 }); },
      async () => { await S.overlayOff(); await S.overlay({ kind: 'toast', top: 80, title: 'Google Kalender', text: 'SPD Soltau – meine Zusagen hinzugefügt' }); },
    ],
    ios: [
      async () => { await S.scroll('#kalender'); },
      async () => { await S.tap('#kalender [data-kal-neu]'); await S.scroll('#kalender'); },
      async () => { await S.point('#kalender .abo-karte a[href^="webcal"]'); },
      async () => { await S.overlay({ kind: 'dialog', title: 'Kalender abonnieren?', text: 'Der Kalender „SPD Soltau – meine Zusagen“ wird zu deinen abonnierten Kalendern hinzugefügt.', buttons: ['Abbrechen', 'Abonnieren'], primary: 1 }); },
    ...
    windows: [
      ...
      async () => { await S.tap('#kalender [data-kal-neu]'); await S.scroll('#kalender'); },
      async () => { await S.tap('#kalender .abo-karte [data-copy]'); },
      async () => { await S.overlay({ kind: 'dialog', top: '36%', width: 460, title: 'Aus dem Internet abonnieren', text: '<div style="border:1px solid #ccc;padding:8px;font-size:14px;color:#333">https://spd-soltau.de/assets/kalender/…ics</div>', buttons: ['Abbrechen', 'Importieren'], primary: 1 }); },
      async () => { await S.overlay({ kind: 'dialog', top: '36%', width: 460, title: 'Per URL – Google Kalender', text: '<div style="font-size:14px;color:#333;margin-bottom:8px">calendar.google.com → Weitere Kalender <b>+</b></div><div style="border:1px solid #ccc;padding:8px;font-size:14px;color:#333">https://spd-soltau.de/assets/kalender/…ics</div>', buttons: ['Abbrechen', 'Kalender hinzufügen'], primary: 1 }); },
      async () => { await S.overlayOff(); await S.overlay({ kind: 'toast', top: 80, title: 'Fertig', text: 'Die Termine laufen jetzt automatisch mit' }); },
    ],
    macos: [
      async () => { await S.scroll('#kalender'); },
      async () => { await S.tap('#kalender [data-kal-neu]'); await S.scroll('#kalender'); },
      async () => { await S.point('#kalender .abo-karte a[href^="webcal"]'); },
      async () => { await S.overlay({ kind: 'dialog', title: 'Kalender abonnieren?', text: 'Der Kalender „SPD Soltau“ wird hinzugefügt. Automatisch aktualisieren: Jede Stunde.', buttons: ['Abbrechen', 'Abonnieren'], primary: 1 }); },
      async () => { await S.overlayOff(); await S.overlay({ kind: 'toast', top: 80, title: 'Kalender', text: 'SPD Soltau – abonniert' }); },
    ],
  } }),
