...
export function makeStatistik(ctx) {
  ...
  async function statistik(panel) {
    ...
    const render = () => {
      ...
      // Zählt dieser Browser mit? Angemeldete Mitglieder zählen nie mit, „?zaehlen=aus“ schaltet es für einen Browser ab
      const merk = k => { try { return !!localStorage.getItem(k); } catch (e) { return false; } };
      const angemeldet = merk('spd-me'), abgeschaltet = merk('spd-nicht-zaehlen');
      panel.innerHTML = `${sectionHead('Statistik', 'Aufrufe der Website – ohne Cookies, ohne IP-Adressen, ohne Personen')}
      <div class="mb-tabs st-zeit">${ZEIT.map(([k, l]) => `<button type="button" class="chip" data-zeit="${k}" aria-pressed="${k === zeit}">${l}</button>`).join('')}<button type="button" class="chip st-csv" data-csv>CSV</button><button type="button" class="chip st-aus" data-mitzaehlen aria-pressed="${abgeschaltet}">Dieser Browser zählt nicht mit</button></div>
      ...
      <p class="small muted st-fuss">Gezählt wird jeder Seitenaufruf mit Seite, Herkunft (nur die Domain), Gerät, Browsersprache und Ladezeit – keine IP-Adresse, keine Kennung, kein Cookie. „Besuche“ sind Aufrufe, die von außen kommen (Adresse eingetippt, Suchmaschine, Link). Der Push-Dienst verdichtet die Einträge alle paar Minuten zu Tageswerten und löscht die Rohdaten. Kommt dieselbe Person später am Tag wieder, ist das ein weiterer Besuch – ohne Kennung lässt sich das nicht zusammenfassen.${seit ? ` Daten seit ${esc(new Date(seit + 'T12:00:00').toLocaleDateString('de-DE', { day: 'numeric', month: 'long', year: 'numeric' }))}.` : ''}</p>
      <p class="small muted st-fuss"><b>Dieser Browser: ${abgeschaltet ? 'zählt nicht mit' : angemeldet ? 'zählt nicht mit, weil du angemeldet bist' : 'zählt mit'}.</b> Wer im Mitgliederbereich angemeldet ist, wird nie mitgezählt. Für einen Browser ohne Anmeldung – zum Beispiel am Arbeitsplatz – einmal <a href="${esc((SPD.base || '') + '/?zaehlen=aus')}" target="_blank" rel="noopener">spd-soltau.de/?zaehlen=aus</a> aufrufen; das merkt sich dieser Browser, rückgängig mit <code>?zaehlen=an</code>.</p>`;
      $$('[data-zeit]', panel).forEach(b => b.addEventListener('click', () => { zeit = b.dataset.zeit; store.set('spd-stat-zeit', zeit); render(); }));
      $('[data-mitzaehlen]', panel)?.addEventListener('click', () => {
        try { if (localStorage.getItem('spd-nicht-zaehlen')) localStorage.removeItem('spd-nicht-zaehlen'); else localStorage.setItem('spd-nicht-zaehlen', '1'); } catch (e) { /* ohne Speicher */ }
        render();
      });
