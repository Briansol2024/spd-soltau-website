...
const SCHEMA = {
  ...
  Ratsvorbereitung: [T('b', 'Bereich/Ausschuss'), T('protokoll', 'Ergebnisse trägt ein (ID)'), T('aktuell', 'Aktueller Punkt', 'NUMBER'), T('typ', 'Art'), T('gremium', 'Gremium'), T('sitzung', 'Sitzung am'), T('zeit', 'Uhrzeit'), T('ort', 'Ort'), T('titel', 'Titel'), T('link', 'Link'), T('hinweis', 'Hinweis'), T('von', 'Von')],
...
const RAT = {
  ...
  SitzungNotizen: { displayName: 'Sitzungen – Notizen (privat)', permissions: P('SITE_MEMBER_AUTHOR', 'SITE_MEMBER', 'SITE_MEMBER_AUTHOR', 'SITE_MEMBER_AUTHOR'), fields: [T('sitzungId', 'Sitzung'), T('name', 'Von'), T('memberId', 'Mitglieds-ID'), T('text', 'Text'), T('skizze', 'Skizze (Striche)')] },
