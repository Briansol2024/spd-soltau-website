...
export function makeFilm(ctx) {
  ...
  function wireProjekt(v, p, overlays) {
    ...
    // Auftrag löschen: Datei (Teile räumt der Dienst ab) und Auftrag – nach dem Herunterladen liegt dann nichts mehr bei Wix
    const auftragWeg = async id => {
      const a = st.auftraege.find(x => x._id === id); if (!a) return;
      const mid = /^material:(.+)$/.exec(a.url || '')?.[1];
      if (mid) { try { await db.remove('FilmMaterial', mid); } catch (e) { /* schon weg oder fremd */ } st.material = st.material.filter(m => m._id !== mid); }
      try { await (st.konto?.db || db).remove('Auftraege', id); } catch (err) { alert('Auftrag nicht gelöscht: ' + errText(err)); return; }
      st.auftraege = st.auftraege.filter(x => x._id !== id); projekt(v, p._id);
    };
    v.addEventListener('click', async e => {
      const w = e.target.closest('[data-auftrag-weg]'); if (!w) return;
      if (!confirm('Diesen Clip samt Datei löschen? Vorher herunterladen, wenn du ihn behalten willst.')) return;
      await auftragWeg(w.dataset.auftragWeg);
    });
    ...
    v.addEventListener('click', async e => {
      ...
      try { await db.remove('FilmMaterial', id); st.material = st.material.filter(m => m._id !== id); const a = st.auftraege.find(x => x.url === 'material:' + id); if (a) { await (st.konto?.db || db).remove('Auftraege', a._id).catch(() => {}); st.auftraege = st.auftraege.filter(x => x._id !== a._id); } projekt(v, p._id); } catch (err) { alert('Nicht entfernt: ' + errText(err)); }
    });
  }
