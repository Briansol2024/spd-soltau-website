...
if "!KEY!"=="" (
  ...
  echo   Im Browser ist die Seite vorbereitet: https://manage.wix.com/account/api-keys
  ...
  echo.
  echo Bitte den Schluessel hier einfuegen (Rechtsklick = Einfuegen) und Enter druecken:
  set /p "KEY=> "
...
schtasks /Create /TN "SPD Soltau Push-Dienst" /SC MINUTE /MO 5 /TR "wscript.exe \"%~dp0Push-Dienst leise.vbs\"" /F || goto :err
echo.
...
:err
echo.
echo Da ist etwas schiefgegangen - Ausgabe oben pruefen. Bei "Admin-Zugang fehlgeschlagen": Schluessel und Berechtigungen kontrollieren
echo (WIX_API_KEY in .env loeschen und dieses Skript erneut starten, um einen neuen Schluessel einzugeben).
pause
exit /b 1
