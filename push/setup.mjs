...
const RAT = {
  ...
  // KI-Agent (SKM-Server): Anfragen aus dem Filmdreh und der Herzschlag des Agenten
  KiAuftraege: { displayName: 'KI-Agent – Anfragen', permissions: P('SITE_MEMBER', 'SITE_MEMBER', 'SITE_MEMBER', 'SITE_MEMBER_AUTHOR'), fields: [T('projektId', 'Projekt'), T('memberId', 'Mitglieds-ID'), T('von', 'Von'), T('wunsch', 'Wunsch'), T('verlauf', 'Gespräch (JSON)'), T('skriptVorher', 'Skript vorher'), T('overlaysVorher', 'Overlays vorher'), T('titel', 'Projekt-Titel'), T('art', 'Art'), T('datum', 'Termin'), T('status', 'Status'), T('antwort', 'Antwort'), T('skript', 'Skript'), T('overlays', 'Overlays (JSON)'), T('hinweis', 'Rückfrage'), T('fehler', 'Fehler'), T('gestartetAm', 'Gestartet'), T('fertigAm', 'Fertig'), T('dauer', 'Dauer (s)', 'NUMBER')] },
  KiStatus: { displayName: 'KI-Agent – Status', permissions: P('SITE_MEMBER', 'ADMIN', 'ADMIN', 'ADMIN'), fields: [T('key', 'Schlüssel'), T('zuletzt', 'Zuletzt gesehen'), T('host', 'Rechner'), T('angemeldet', 'Claude angemeldet', 'BOOLEAN'), T('version', 'Version'), T('info', 'Info')] },
  // Filmdreh (Regie-Modus): Projekte, Material (Dateien landen über den Dienst im Medienmanager), Dateiteile beim Hochladen
  FilmProjekte: { displayName: 'Filmdreh – Projekte', permissions: P('SITE_MEMBER', 'SITE_MEMBER', 'SITE_MEMBER', 'SITE_MEMBER'), fields: [T('titel', 'Titel'), T('art', 'Art'), T('status', 'Status'), T('datum', 'Termin'), T('skript', 'Skript'), T('drehplan', 'Drehplan'), T('notizen', 'Notizen'), T('overlays', 'Overlays (JSON)'), T('ki', 'KI-Unterhaltung (JSON)'), T('von', 'Von'), T('memberId', 'Mitglieds-ID')] },
  ...
  FilmTeile: { displayName: 'Filmdreh – Dateiteile (kurzlebig)', permissions: P('ADMIN', 'SITE_MEMBER', 'ADMIN', 'ADMIN'), fields: [T('materialId', 'Material'), T('nr', 'Nr.', 'NUMBER'), T('daten', 'Daten (Base64)')] },
