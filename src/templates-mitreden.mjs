...
export function mitredenBlock(d) {
  ...
  <div class="wrap section" id="mitreden">
    <div class="section-head" style="margin-bottom:24px"><h2 class="title">Mitreden</h2><span class="small muted">Vier Wege, ohne Parteibuch – alles öffentlich nachlesbar</span></div>
    <div class="quick quick-mitreden">
      ...
      <a href="${url('/mitreden/fragen/')}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9.5 9a2.5 2.5 0 0 1 5 0c0 2-2.5 2-2.5 4M12 17h.01"/><circle cx="12" cy="12" r="10"/></svg><b>Fragen Sie uns</b><small>Jede Frage bekommt eine Antwort im Video – auch anonym.</small>${m.fragen.length ? zahl(m.fragen.length, 'beantwortet') : ''}</a>
      <a href="${url('/mitreden/abstimmung/')}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 11l3 3 8-8"/><path d="M20 12v7a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h9"/></svg><b>Sie entscheiden mit</b><small>${offen ? esc(offen.frage) : 'Kleine Abstimmungen – das Ergebnis geht in den Ausschuss.'}</small>${offen ? zahl(stimmen, 'Stimmen') : ''}</a>
...
export function mitredenPage(d) {
  return `${pageHead('Ohne Parteibuch', 'Mitreden', 'Vier Wege, sich einzumischen – und alles öffentlich nachlesbar: Anliegen unterstützen, Fragen stellen, abstimmen, Baustellen verstehen.')}
  ${videoBlock(d)}
  ${mitredenBlock(d)}`;
}
...
export function anliegenPage(d) {
  ...
  return `${pageHead('Was Soltau bewegt', 'Anliegen, die<br>viele teilen', 'Anliegen aus der Stadt, sortiert danach, wie viele sagen: Das betrifft mich auch. Kein Bürgerentscheid – ein Stimmungsbild. Die drei meistunterstützten nehmen wir jeden Monat mit in die Fraktionssitzung und schreiben hier, was wir erreicht haben.')}
  <div class="wrap section">
    ...
    <p class="small muted">Veröffentlicht wird nur mit Ihrem Einverständnis (Häkchen im Formular), ohne Namen und ohne Adresse. Sie bekommen eine Nachricht, sobald sich etwas tut.</p>
  </div>`;
}
...
export function fragenPage(d) {
  ...
  return `${pageHead('Fragen Sie uns', 'Jede Frage<br>bekommt eine Antwort', 'Im Video, spätestens am Monatsende – auch anonym. Was viele fragen, kommt zuerst dran.')}
  <div class="wrap section">
    <form class="form wix-form frage-form" id="form-frage" data-collection="Fragen" novalidate>
      ...
      <div class="form-fields">
        ...
        <p class="small muted">Wir antworten persönlich (wenn Sie eine E-Mail angeben) und öffentlich in der nächsten Fragestunde. Datenschutz: nur Frage und Vorname werden veröffentlicht.</p>
      </div>
      <p class="form-ok" hidden><b>Danke – Ihre Frage ist drin.</b> Wir antworten in der nächsten Fragestunde; mit E-Mail-Adresse auch persönlich.</p>
    </form>
    ...
    <div class="fragen-liste" id="fragen-liste" data-typ="frage">
      ...
      </article>`).join('') : '<p class="muted">Die ersten Antworten kommen mit der nächsten Fragestunde.</p>'}
    </div>
  </div>`;
}
...
export function abstimmungPage(d) {
  ...
  return `${pageHead('Sie entscheiden mit', 'Eine Frage,<br>Ihre Kreuze', 'Kleine Abstimmungen zu Dingen, die wir wirklich beeinflussen können. Das Ergebnis nehmen wir mit in den Ausschuss – und hier steht hinterher, was daraus wurde.')}
  <div class="wrap section">
...
export function baustellenPage(d) {
  ...
  return `${pageHead('Wo wird gebaut?', 'Baustellen<br>und Sperrungen', 'Was gerade gebaut oder gesperrt ist – und warum, wie lange, mit welcher Umleitung. Aus den Bekanntmachungen der Stadt, ergänzt um unsere Einordnung.')}
  <div class="wrap section">
    ...
    <div class="baustellen-liste" id="baustellen-liste">
      ...
      <article class="baustelle" data-id="${esc(b.id)}" data-art="${esc(b.art)}" style="--pf:${ART_FARBE[b.art] || '#E3000F'}">
        <div class="baustelle-kopf"><span class="rang">${i + 1}</span><div><h3>${esc(b.titel)}</h3><span class="small muted">${esc(b.art)}${b.bis ? ' · ' + esc(b.bis) : ''}</span></div></div>
        <details class="anliegen-mehr"><summary>Was, warum, Umleitung</summary>
          ...
          ${b.spd ? `<p class="spd-antwort"><b>SPD Soltau:</b> ${esc(b.spd)}</p>` : ''}${b.quelle ? `<p class="small muted">Quelle: ${esc(b.quelle)}</p>` : ''}
        </details>
